...
import { Bell, Search, User as UserIcon, Menu, Store, Sun, Moon } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ user, config, isSidebarCollapsed, setIsSidebarCollapsed }) => {
  ...
  return (
    <header className="h-20 glass border-b border-border px-4 md:px-8 flex items-center justify-between sticky top-0 z-40">
      ...
      <div className="hidden lg:flex items-center bg-surface/40 rounded-full px-5 py-2.5 border border-border w-96 group focus-within:border-accent/40 transition-all duration-300">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        {/* Theme switch — flips the whole app between the black+gold identity
            and the light background (same gold/action colours in both). */}
        <button
          onClick={toggleTheme}
          title={isLight ? 'Passer en mode sombre' : 'Passer en mode clair'}
          aria-label={isLight ? 'Passer en mode sombre' : 'Passer en mode clair'}
          className="relative w-16 h-9 rounded-full bg-surface-2 border border-border flex items-center px-1 transition-colors duration-300 hover:border-accent/50 shrink-0"
        >
          <motion.span
            layout
            transition={{ type: 'spring', stiffness: 500, damping: 32 }}
            className={cn(
              'w-7 h-7 rounded-full flex items-center justify-center shadow-md bg-gradient-to-br from-accent to-accent-light text-on-accent',
              isLight ? 'ml-auto' : 'ml-0'
            )}
          >
        ...
        <div className="relative">
          <button
            ...
            className={cn(
              ...
              showNotifications ? "bg-accent text-on-accent shadow-lg shadow-accent/20" : "text-ink/40 hover:text-accent hover:bg-accent/5"
            )}
          >
            <Bell size={22} />
            <span className="absolute top-2.5 right-2.5 w-2.5 h-2.5 bg-accent rounded-full border-2 border-border shadow-sm animate-pulse"></span>
          </button>
            ...
            {showNotifications && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-80 bg-surface rounded-3xl shadow-2xl border border-border overflow-hidden z-50"
                >
                  <div className="p-5 border-b border-border flex justify-between items-center bg-primary-bg/30">
                    ...
                    <button 
                      onClick={() => setShowNotifications(false)}
                      className="text-[10px] bg-accent/10 text-accent hover:bg-accent hover:text-on-accent px-2 py-1 rounded-full font-bold transition-all"
                    >
        ...
        <div className="flex items-center gap-3 pl-4 md:pl-6 border-l border-border">
          ...
          <div className="w-11 h-11 rounded-full border-2 border-accent/20 p-0.5 shadow-sm hover:border-accent/40 transition-colors cursor-pointer overflow-hidden bg-surface">
            {user.avatar ? (
